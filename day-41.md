...
services:
  ...
  app:
    ...
    depends_on:
      postgres: { condition: service_healthy }   # chờ DB healthy mới start
...
1. Viết `Dockerfile` multi-stage (JDK build → JRE runtime), dùng **layered jar**, chạy bằng user non-root, đặt `-XX:MaxRAMPercentage=75.0`.
2. Viết `.dockerignore` loại `target/`, `.git/`, `.env`...
3. Viết `docker-compose.yml` gồm `app` + `postgres` + `redis` + `kafka`, mỗi service có **healthcheck**, app `depends_on` các service ở `condition: service_healthy`.
4. Cấu hình app đọc host từ **env var** (tên service), `mem_limit: 512m` cho app.
5. Chạy `docker compose up -d`, xác nhận tất cả **healthy**, gọi thử `POST /auctions/{id}/bids` thấy ghi DB + cập nhật Redis leaderboard + phát Kafka event hoạt động xuyên suốt.
