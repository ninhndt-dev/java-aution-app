    ...
    public static void startWatchdog() {
        ...
        Thread watchdog = new Thread(() -> {
            while (!Thread.currentThread().___()) { // Điền phương thức kiểm tra cờ ngắt
                long[] deadlocked = mx.___(); // Điền phương thức tìm các thread bị deadlock
                if (deadlocked != null) {
...
public class TryLockDemo {
    ...
    static void doWork(ReentrantLock first, ReentrantLock second) throws InterruptedException {
        while (true) {
            ...
            try {
                gotFirst = first.___(200, TimeUnit.MILLISECONDS); // Điền phương thức thử lấy khóa có timeout
                if (gotFirst) {
                    gotSecond = second.___(200, TimeUnit.MILLISECONDS); // Điền phương thức thử lấy khóa có timeout
                }
                ...
                if (gotSecond) second.___(); // Điền phương thức nhả khóa
                if (gotFirst)  first.___(); // QUAN TRỌNG: luôn nhả những gì đã giữ
            }
...
public class BuggyBank {
    ...
    public void transfer(Account from, Account to, long amount) {
        ___ (from.lock()) { // Điền từ khóa đồng bộ
            ___ (to.lock()) { // T2: transfer(Y,X) khóa Y rồi xin X
...
public class SafeBank {
    ...
    public boolean transfer(Account from, Account to, long amount) {
        ...
        ___ (first.lock()) { // Điền từ khóa đồng bộ
            ___ (second.lock()) { // luôn cùng hướng id nhỏ → lớn
