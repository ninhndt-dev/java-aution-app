...
server:
  port: 8080
  shutdown: ___         # Điền chế độ tắt êm (đợi các request hiện tại xong rồi mới tắt)
management:
  endpoints:
    web:
      exposure:
...
spring:
  datasource:
    ...
    url: ${___} # Điền Tên Biến cho DB URL
    username: ${___} # Điền Tên Biến cho DB USER
    password: ${___} # Điền Tên Biến cho DB PASSWORD
    hikari:
...
public class BidExpiryWorker {
    ...
    /**
     * Spring gọi method này khi context đóng (lúc graceful shutdown).
     * Ta set cờ để vòng lặp thoát sạch, không cắt ngang.
     */
    @___ // Điền Annotation lắng nghe sự kiện Bean bị hủy
    public void stop() {
...
USER appuser
EXPOSE 8080
___ ["java", \ # Điền từ khóa khai báo điểm vào của image
  "-XX:___=75.0", \ # Điền tham số để set giới hạn RAM theo phần trăm của container
  "-XX:+UseG1GC", \
...
jobs:
  build-and-test:
    ...
    steps:
      ...
      - name: Build & test
        run: ./mvnw clean ___   # Điền phase maven để chạy test tích hợp + build

  # (Phần build-image và deploy tham khảo template gốc nếu cần)
```
    ...
    spec:
      containers:
        - name: auction-api
          ...
          livenessProbe:
            httpGet:
              path: /actuator/health/___ # Điền path của liveness
              port: 8080
          ...
          readinessProbe:
            httpGet:
              path: /actuator/health/___ # Điền path của readiness
              port: 8080
            initialDelaySeconds: 10
            periodSeconds: 5
```

### Bước 7 — CHALLENGE: Kịch bản Rollback

> 🏆 Yêu cầu:
> 1. Viết 1 file `rollback-plan.md`.
> 2. Đặt tình huống: Bạn vừa code thêm 1 tính năng, thêm cột `status` vào bảng `auctions`. Khi Deploy lên, code lỗi và làm App chết liên tục.
> 3. Nêu các bước để quay lại bản ổn định trước đó (rollback image, và rollback schema thế nào nếu `status` lỡ bị thêm vào).
...
1. **Tách cấu hình theo môi trường:** giữ `application.yml` cho dev, tạo `application-prod.yml` theo hướng dẫn điền khuyết với mọi giá trị nhạy cảm (`DB_URL`, `DB_USERNAME`, `DB_PASSWORD`, `JWT_SECRET`) đọc qua `${ENV}`.
2. **Graceful shutdown:** điền `@PreDestroy` vào task nền (nếu có). Bật `server.shutdown: graceful` + `spring.lifecycle.timeout-per-shutdown-phase: 30s`.
3. **Đóng gói:** điền các từ khóa còn thiếu trong `Dockerfile` (như `ENTRYPOINT`, `-XX:MaxRAMPercentage`). Build image và chạy container thử.
4. **CI/CD:** điền `verify` vào GitHub Actions để chạy test mỗi lần push.
5. **Probe:** điền `liveness` và `readiness` cho Kubernetes.
6. Hoàn thành **CHALLENGE** ở Bước 7: Viết kịch bản Rollback xử lý sự cố.

**Kết quả mong đợi:** Code đã sẵn sàng đưa lên môi trường thật, quản lý cấu hình chuẩn 12-factor, đóng gói image bảo mật với tham số bộ nhớ tối ưu, hỗ trợ giám sát và tự phục hồi thông qua Kubernetes.
...
- [ ] Hoàn thành Challenge: Viết kịch bản Rollback
