...
<dependency>
  ...
  <artifactId>___</artifactId> <!-- Điền dependency bắt buộc để có các endpoint /health, /metrics -->
</dependency>
<dependency>
...
management:
  endpoints:
    web:
      exposure:
        include: ___   # Điền danh sách các endpoint CẦN THIẾT được phép gọi qua HTTP (phân cách dấu phẩy)
  endpoint:
    health:
...
public class BidMetrics {
    ...
    public BidMetrics(MeterRegistry registry) {
        ...
        this.accepted = registry.___( "auction.bids", "result", "accepted"); // Điền method đếm số (luôn tăng)
        this.rejected = registry.___( "auction.bids", "result", "rejected");
        this.placeBidTimer = registry.___( "auction.placeBid.latency"); // Điền method đo thời lượng
...
@Component
public class RedisHealthIndicator implements ___ { // Điền interface bắt buộc cho custom health check
    ...
    public Health health() {
        try {
            ...
            return "PONG".equalsIgnoreCase(pong)
                ? Health.___().withDetail("redis", "reachable").build() // Điền method khi mọi thứ OK
                : Health.___().withDetail("redis", "unexpected: " + pong).build(); // Điền method khi có sự cố
        } catch (Exception e) {
...
management:
  endpoint:
    health:
      group:
        ___: # Nhóm readiness (Sẵn sàng nhận traffic)
          include: readinessState, db, redis    
        ___: # Nhóm liveness (Còn sống không)
          include: livenessState                  # (không phụ thuộc DB)
```
...
scrape_configs:
  - job_name: 'auction-api'
    metrics_path: '___' # Điền đường dẫn của Prometheus Endpoint từ Actuator
    scrape_interval: 15s
    static_configs:
      - targets: ['app:8080']        # tên service trong docker-compose (Day 41)
```

### Bước 7 — CHALLENGE: Lên Grafana

> 🏆 Yêu cầu:
> 1. Dùng Docker Compose dựng 1 container Prometheus và 1 container Grafana lên (trỏ Prometheus về `host.docker.internal:8080`).
> 2. Truy cập Grafana, tạo 1 Dashboard dùng dữ liệu Prometheus.
> 3. Hiển thị đồ thị đo đạc số phiên đấu giá đang diễn ra (từ gauge `auction.open.sessions`).
> 4. Hiển thị tốc độ đặt Bid mỗi phút bằng PromQL: `rate(auction_bids_total{result="accepted"}[1m]) * 60`
...
1. Thêm Actuator + Prometheus registry; thêm vào config expose `health,info,metrics,prometheus`, bật `health.probes`.
2. Tạo `BidMetrics` theo form điền khuyết: thêm `.counter`, `.timer`. Gắn vào `BidService.placeBid`.
3. Viết `RedisHealthIndicator` kế thừa `HealthIndicator`; để DB + Redis vào **readiness group**, `livenessState` riêng (không phụ thuộc DB).
4. Khởi động lại Server, gọi thử `http://localhost:8080/actuator/prometheus` để xem nó in ra metric dạng text.
5. Cấu hình `prometheus.yml` (nối docker-compose Day 41), thêm service `prometheus` + `grafana` vào compose.
6. Hoàn thành **CHALLENGE** ở Bước 7: Dựng Grafana, Dashboard, tạo biểu đồ PromQL số lượng Bid/phút, Dashboard phải nhảy real-time khi test Postman.

**Kết quả mong đợi:** Grafana hiển thị số bid/phút và latency của `placeBid` theo thời gian thực; `/actuator/health/readiness` chuyển DOWN khi tắt Redis/DB (orchestrator ngừng route traffic) còn `/liveness` vẫn UP (không bị restart oan); mọi log của một request gom được theo traceId.
...
- [ ] Hoàn thành Challenge: Dựng Grafana Dashboard
