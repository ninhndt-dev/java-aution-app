...
___ Notifier { // Điền từ khóa khai báo giao diện
    void send(String message);                 // abstract: bắt buộc cài

    // default method: có sẵn thân hàm -> lớp implement KHÔNG cần override
    ___ void sendUrgent(String message) { // Điền từ khóa cho phép viết thân method trong interface
        send("[KHẨN] " + message);             // tái dùng abstract method
    }
}

class EmailNotifier ___ Notifier { // Điền từ khóa thực thi giao diện
    @Override
...
___ // Điền annotation để đảm bảo chỉ có 1 abstract method
interface BidValidator {
    ...
    ___ BidValidator min(long floor) { // Điền từ khóa khai báo method tĩnh trong interface
...
___ class AbstractAuction { // Điền từ khóa khai báo lớp trừu tượng
...
class EnglishAuction ___ AbstractAuction { // Điền từ khóa kế thừa lớp trừu tượng
