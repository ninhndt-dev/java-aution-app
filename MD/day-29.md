...
### Bước 1 — Aspect chặn request với `@Around` (Rate Limiting)

Ta sẽ tạo một Aspect để đếm số lần gọi method `placeBid`. Nếu vượt giới hạn, chặn luôn không cho method thật chạy.
...
@___ // Đánh dấu class này là một Aspect
@___ // Đăng ký nó thành bean
public class RateLimitAspect {

    private final Map<String, Integer> callCounts = new ConcurrentHashMap<>();

    // Pointcut: Bất cứ method nào được đánh dấu @RateLimit
    @___("@annotation(rateLimit)")
    public Object enforceRateLimit(ProceedingJoinPoint pjp, RateLimit rateLimit) throws Throwable {
        // Lấy tên bidder (tham số thứ 2 của hàm placeBid)
        String bidder = (String) pjp.getArgs()[1]; 
        int current = callCounts.getOrDefault(bidder, 0);

        if (current >= rateLimit.maxCalls()) {
            ___ ___ RuntimeException("TooManyBidsException: " + bidder + " đã đặt quá nhiều!"); // Điền từ khóa ném ngoại lệ và khởi tạo
        }

        callCounts.put(bidder, current + 1);
        
        // BẮT BUỘC gọi proceed() để method gốc chạy
        return pjp.___(); 
    }
}
```

Trong `BidService`, gắn annotation:
```java
...
### Bước 2 — Đo thời gian và tạo Endpoint Debug (Metrics)
...
@Aspect
@Component
@Order(2) // Đảm bảo chạy SAU RateLimitAspect
public class TimingAspect {

    public static final AtomicLong totalCalls = new AtomicLong(0);

    // Bắt mọi hàm trong class kết thúc bằng Service
    @Around("execution(public * com.example.auction..*Service.*(..))")
    public Object measure(ProceedingJoinPoint pjp) ___ Throwable { // Điền từ khóa khai báo ném ngoại lệ
        long start = System.nanoTime();
        ___ { // Điền từ khóa bắt đầu khối try
            totalCalls.incrementAndGet();
            return pjp.proceed();
        } ___ { // Điền từ khóa thực thi cuối cùng (finally)
            long ms = (System.nanoTime() - start) / 1_000_000;
            System.out.println("[timing] " + pjp.getSignature().getName() + " mất " + ms + " ms");
        }
    }
}
```

Tạo endpoint kiểm tra trong `DebugController` (từ Day 28):
```java
    @GetMapping("/metrics")
    public String getMetrics() {
        return "Tổng số lần gọi Service: " + TimingAspect.totalCalls.get();
    }
```

### Bước 3 — Kiểm tra kết quả bằng lệnh `curl`

Hãy dùng vòng lặp để gọi API liên tiếp 6 lần. Lần thứ 6 sẽ bị chặn bởi Aspect.
```bash
# Đặt 6 bid liên tiếp -> bid cuối bị chặn
for i in {1..6}; do
  echo "Lần $i:"
  curl -s -X POST http://localhost:8080/api/auctions/1/bids \
    -H "Content-Type: application/json" \
    -d '{"bidder":"Alice","amount":'$((i*100000))'}'
  echo ""
done

# Kiểm tra metrics
curl http://localhost:8080/api/debug/metrics
# Expected: Tổng số lần gọi Service: 5 (do lần 6 bị chặn TRƯỚC khi vào service)
```

### Bước 4 — CHALLENGE: Tự động Retry khi lỗi (Thử thách)

> 🏆 Yêu cầu:
> 1. Tạo annotation `@RetryOnFailure(maxRetries=3)`.
> 2. Viết `RetryAspect` với `@Around`. Nếu method ném ra `RuntimeException`, Aspect sẽ tự động bắt lỗi bằng vòng lặp `for` và `try-catch`, in ra dòng log "Đang thử lại...", rồi gọi `pjp.proceed()` một lần nữa. Tối đa 3 lần. Nếu vẫn lỗi, ném lỗi ra ngoài.
> 3. Cập nhật `AuctionController`: Tạo một endpoint `GET /api/debug/flaky` và ném ra một exception ngẫu nhiên (50% cơ hội lỗi). Gắn `@RetryOnFailure` vào hàm trong Service.
> 4. Gọi API bằng `curl` và quan sát log của Spring Boot xem aspect có thực sự tự động thử lại không!
...
> Hôm nay ta thêm **observability, rate limit + auto-retry** cho luồng đấu giá bằng AOP, giữ code nghiệp vụ sạch.

**Nhiệm vụ Day 29:**

1. Điền các chỗ trống `___` trong code thực hành ở trên.
2. Thêm dependency `spring-boot-starter-aop` (nếu chưa có).
3. Viết `RateLimitAspect` (`@Around`) chặn request thứ 6 của cùng một `bidder` (Bước 1). Gắn `@RateLimit` vào `BidService.placeBid`.
4. Viết `TimingAspect` đo thời gian gọi mọi method trong các `*Service` (Bước 2).
5. Expose `GET /api/debug/metrics` trả về tổng số lần gọi hàm để kiểm tra.
6. Dùng lệnh `curl` trong vòng lặp bash để test rate limit (Bước 3).
7. Hoàn thành **CHALLENGE** ở Bước 4: Viết `@RetryOnFailure` xử lý auto-retry khi gặp lỗi ngẫu nhiên.
8. Thêm `@Order` để xác định thứ tự chạy giữa `RateLimitAspect` và `TimingAspect`.
9. Ghi `notes/day-29.md`: đối chiếu `@Around` với middleware Laravel (`$next($request)` ↔ `pjp.proceed()`); giải thích vì sao tổng số hàm trong `/metrics` không tăng khi rate limit kích hoạt.

> 🎯 Tiêu chí đạt: method `placeBid` sạch sẽ; `curl` vòng lặp trả về lỗi ở lần 6; `/metrics` hiển thị đúng số lần; Challenge hoạt động tự retry thành công.
...
- [ ] Dựng `RateLimitAspect` và test chặn request bằng vòng lặp `curl`
- [ ] Hoàn thành Challenge: Tự viết `@RetryOnFailure` Aspect
- [ ] Hoàn thành Mini Project Day 29
