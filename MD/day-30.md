...
EntityManager  ── nhẹ, MỘT cho mỗi transaction/đơn vị công việc
        │  quản lý
        ▼
Persistence Context  ── "không gian làm việc" chứa các entity đang quản lý (Day 33)
```
...
> 💡 **Luôn để quan hệ `@ManyToOne`/`@OneToMany` là `LAZY`** (mặc định `@ManyToOne` là EAGER — nên đặt lại LAZY rõ ràng) để tránh kéo cả cây dữ liệu. Vấn đề lazy/N+1 sẽ học kỹ ở Day 33.
...
| Eager load | `Auction::with('bids')->get()` | `@EntityGraph` / `JOIN FETCH` (Day 33) |
...
@___                                 // Điền annotation ánh dấu class này map với một bảng trong Database
@___(name = "users")                 // Điền annotation và tên bảng tương ứng
public class User {
    @___                             // Điền annotation xác định đây là Khóa chính (Primary Key)
    @___(strategy = GenerationType.IDENTITY) // Điền annotation xác định Khóa chính tự tăng (AUTO_INCREMENT)
    private Long id;

    @___(nullable = false, unique = true) // Điền annotation map với một cột cụ thể
...
public class Auction {
    ...
    @___(EnumType.STRING) // Điền annotation để lưu giá trị Enum dưới dạng Chuỗi thay vì Số (Ordinal)
    @Column(nullable = false)
    ...
    // Quan hệ 1 Phiên Đấu giá (Auction) có nhiều Lượt trả giá (Bid)
    @___(mappedBy = "auction", cascade = CascadeType.ALL, orphanRemoval = true) // Điền annotation thể hiện quan hệ Một-Nhiều
    private List<Bid> bids = new ArrayList<>();

    // LƯU Ý RẤT QUAN TRỌNG:
    // Mọi Entity bắt buộc phải có một Constructor không tham số
    protected Auction() {}
...
public class Bid {
    ...
    // Quan hệ Nhiều Lượt trả giá (Bid) thuộc về 1 Phiên Đấu giá (Auction)
    @___(fetch = FetchType.___) // Điền annotation thể hiện quan hệ Nhiều-Một và loại Fetch (Nạp) lười biếng để tránh N+1 Query
    @___(name = "auction_id", nullable = false) // Điền annotation xác định cột khóa ngoại trong Database
...
// Extends JpaRepository để được cung cấp sẵn các hàm CRUD chuẩn (như save, findById, findAll, delete)
public interface AuctionRepository extends ___(Auction, Long) { // Điền interface Spring Data chuẩn 
    ...
    @___("SELECT a FROM Auction a WHERE a.endsAt < :now AND a.status = 'OPEN'") // Điền annotation để tự viết câu JPQL
    List<Auction> findExpiredButOpen(@Param("now") Instant now);
}
...
public class AuctionService {
    ...
    @___ // Điền annotation để gom tất cả thay đổi Database của hàm này vào một Transaction thống nhất
    public Bid placeBid(Long auctionId, String username, long amount) {
        ...
        auctionRepository.save(auction);     // (thực ra dirty checking tự lo — xem Day 33)
        return bid;
    }
}
```
...
### Bước 5 — CHALLENGE: DTO Projection và theo dõi Hibernate Logs

> 🏆 Yêu cầu:
> 1. Thêm một DTO tên là `AuctionSummary(Long id, String title, long currentPrice)` kiểu Java Record.
> 2. Trong `AuctionRepository`, thêm method `@Query` để query bằng Constructor Expression (chỉ lấy đúng 3 cột).
> 3. Trong `application.yml`, đảm bảo đã bật `spring.jpa.show-sql=true`.
> 4. Chạy hàm query vừa tạo, và kiểm tra cửa sổ Log của IDE để xem Hibernate sinh ra đúng câu SQL SELECT 3 cột hay là nó SELECT toàn bộ bảng `auctions`? Bằng cách này, bạn hiểu được giá trị tối ưu của DTO Projection.
...
- **Trả entity ra API** → lỗi serialize vòng lặp + `LazyInitializationException` (Day 33). Dùng DTO/projection.
...
- **Transaction biên giới rõ ràng**: `@Transactional` ở tầng service (Day 33 đào sâu persistence context gắn với transaction).
...
1. Tạo entity `User`, `Auction`, `Bid` với mapping quan hệ: `Auction 1—N Bid`, `Bid N—1 User`, `Bid N—1 Auction` (Bài 1).
2. Tạo `AuctionRepository`, `BidRepository`, `UserRepository` (`extends JpaRepository`) với các query params căn bản (Bài 2).
3. Chuyển `AuctionService.placeBid` sang dùng repository và nhớ bật `@Transactional` (Bài 3).
4. Cấu hình DB (PostgreSQL/MySQL), đặt `ddl-auto: validate` + Flyway tạo schema; chạy thử CRUD (Bài 4).
5. Hoàn thành **CHALLENGE** ở Bước 5: Cấu hình `spring.jpa.show-sql=true` và quan sát câu lệnh JPQL trả về DTO Projection có SELECT chính xác những trường cần thiết không.

> 🎯 Tiêu chí đạt: Các bảng tự động ánh xạ đúng kiểu, các câu Repository thực thi chuẩn qua Log Console. Cấu trúc Transactional không bị sai khi Save.
...
- [ ] Implement Entity (User, Auction, Bid)
- [ ] Set up JPA Repository với Query Method
- [ ] Triển khai Transaction cho Data Mutation
- [ ] Setup kết nối DB và cấu hình Log SQL
- [ ] Hoàn thành Challenge: Inspect Hibernate Query bằng DTO Projection
