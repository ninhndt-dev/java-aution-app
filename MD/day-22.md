...
public class FixedPoolDemo {
    ...
    public static void main(String[] args) {
        ...
        try (___ pool = Executors.___(4)) { // Điền interface và phương thức tạo fixed pool
            for (int i = 1; i <= 20; i++) {
                final Bid bid = new Bid("user-" + i, i * 100L);
                pool.___(() -> process(bid));   // Điền phương thức nộp Runnable task
            }
...
public class CallableFutureDemo {
    public static void main(String[] args) throws Exception {
        try (ExecutorService pool = Executors.newFixedThreadPool(3)) {
            ...
            ___<Long> task = () -> { // Điền interface trả về kết quả
                Thread.sleep(100);
                return ThreadLocalRandom.current().nextLong(1000);   // "tính giá đề xuất"
            };

            List<___<Long>> futures = new ArrayList<>(); // Điền interface nhận kết quả tương lai
            for (int i = 0; i < 5; i++) futures.add(pool.submit(task));

            long sum = 0;
            for (___<Long> f : futures) { // Điền interface
                sum += f.___();          // Điền phương thức lấy kết quả (blocking)
            }
...
public class CustomPoolDemo {
    public static void main(String[] args) throws InterruptedException {
        ...
        ___ pool = new ___( // Điền class triển khai ThreadPool chuẩn
                2,                                  // corePoolSize
                4,                                  // maximumPoolSize
                30, TimeUnit.SECONDS,               // keepAlive cho thread vượt core
                new ___(10),       // Điền class hàng đợi có kích thước giới hạn
                factory,
                new ___()   // Điền policy để caller tự chạy khi quá tải
        );
        ...
        // Shutdown chuẩn
        pool.___(); // Điền phương thức shutdown an toàn
        if (!pool.___(60, TimeUnit.SECONDS)) { // Điền phương thức chờ shutdown
            pool.___(); // Điền phương thức ép shutdown
        }
...
public class BidProcessor {
    ...
    public BidProcessor() {
        ...
        this.pool = new ___( // Điền class triển khai ThreadPool chuẩn
                4, 8, 30, TimeUnit.SECONDS,
                new ___(1000), // Điền class hàng đợi có kích thước giới hạn
    ...
    public void shutdown() throws InterruptedException {
        pool.___(); // Điền phương thức shutdown an toàn
        if (!pool.___(30, TimeUnit.SECONDS)) pool.___(); // Điền phương thức chờ và ép shutdown
    }
}
```
...
public class BidProcessorDemo {
    public static void main(String[] args) throws Exception {
        ...
        try (___ clients = Executors.___(20)) { // Điền interface và phương thức tạo pool
            ___ done = new ___(N); // Điền class Countdown
            for (int i = 1; i <= N; i++) {
                final long price = i;                      // giá tăng dần 1..N
                clients.___(() -> { // Điền phương thức nộp task
                    proc.submitBid(new BidProcessor.Bid("u" + price, price));
                    done.___(); // Điền phương thức giảm count
                });
            }
            done.___();                                  // Điền phương thức chờ (đã nộp xong toàn bộ bid)
        }
