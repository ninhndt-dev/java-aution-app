...
### Bước 1 — Dựng Auction API bằng Spring Boot từ đầu

Để thấy "phép màu" của Spring Boot ngay lập tức, ta sẽ tạo một REST API đơn giản.

1. Tạo project Spring Boot 3 (Java 21) qua [start.spring.io]
   - Dependencies: `Spring Web`, `Spring Boot DevTools`
   - Group: `com.example`, Artifact: `auction-api`

2. Viết code cho `AuctionController` (hãy điền vào chỗ trống `___`):
...
@___ // Đánh dấu đây là một REST Controller (bean xử lý web)
public ___ AuctionController { // Điền từ khóa khai báo lớp

    @___("/api/health") // Ánh xạ GET request tới endpoint này
    public String healthCheck() {
        return "{\"status\": \"UP\", \"timestamp\": " + System.currentTimeMillis() + "}";
    }

    @___("/api/auctions")
    public ___<String> listAuctions() { // Điền kiểu dữ liệu danh sách
        // Hardcode tạm 3 phiên đấu giá
        return ___.of("Đấu giá iPhone 15", "Đấu giá MacBook Pro", "Đấu giá Airblade"); // Điền class tạo danh sách tĩnh
    }
}
```

3. **Kiểm tra kết quả:** Chạy ứng dụng (`./mvnw spring-boot:run` hoặc run trong IDE) và mở terminal khác để test:

```bash
curl http://localhost:8080/api/health
# Expected: {"status": "UP", "timestamp": 1718000000}

curl http://localhost:8080/api/auctions
# Expected: ["Đấu giá iPhone 15","Đấu giá MacBook Pro","Đấu giá Airblade"]
```

### Bước 2 — Container "chay" để hiểu rõ bản chất IoC

Spring Boot làm mọi thứ quá tự động. Ta dựng một container tối giản để **nhìn rõ** cơ chế.
...
public ___ PricingStrategy { // Điền từ khóa khai báo interface
    long startingPrice(long basePrice);
}
```

Hãy hoàn thiện class `StandardPricingStrategy` và `AuctionService`:
...
@___   // Đăng ký class này làm bean
...
@___   // Đánh dấu đây là bean tầng nghiệp vụ
public class AuctionService {

    private final PricingStrategy pricingStrategy;

    // Spring tự đưa bean PricingStrategy vào đây
...
public class Main {
    public static void main(String[] args) {
        // 1) Tạo IoC container
        var context = ___ AnnotationConfigApplicationContext(AppConfig.class); // Điền từ khóa khởi tạo đối tượng

        // 2) Lấy bean (giống app()->make() bên Laravel)
        AuctionService service = context.getBean(___.class); // Điền class cần lấy
        service.openAuction(1_000_000);

        // 3) Chứng minh singleton
        AuctionService a = context.getBean(AuctionService.class);
        AuctionService b = context.getBean(AuctionService.class);
        System.out.println("Cùng instance? " + (a == b)); // Expected: true
...
**Kết quả mong đợi:** in ra giá khởi điểm và `Cùng instance? true`.

### Bước 3 — Khai báo bean bằng `@Bean` (cho class không thể sửa)

Giả sử bạn cần một `Clock` (lớp JDK, không sửa được) để đo thời gian:
...
@___   // tên bean mặc định = "clock"
public Clock clock() {
    return Clock.systemDefaultZone(); // tự kiểm soát cách tạo
}
```

Khi đó `context.getBean(Clock.class)` trả về object bạn vừa tạo. Tương đương `$this->app->singleton(Clock::class, ...)` của Laravel.

### Bước 4 — CHALLENGE: Tự giải quyết vấn đề wiring (Thử thách)

> 🏆 Tạo `PricingStrategy` interface với 2 implementation: `StandardPricingStrategy` và `VipPricingStrategy`. Cả hai đều gắn `@Component`.
> 1. Chạy thử xem Spring báo lỗi gì? (Sẽ là `NoUniqueBeanDefinitionException`).
> 2. **Không dùng `@Qualifier`**, làm sao để Spring tự chọn đúng `VipPricingStrategy` để tiêm vào `AuctionService`? 
> *(Gợi ý: Thử dùng `@Primary` hoặc đổi tên biến truyền vào constructor cho khớp với tên bean).*
...
> Từ hôm nay, Auction API bắt đầu **chuyển mình sang Spring**. Ta sẽ dựng container và có những endpoint HTTP đầu tiên chạy được.

**Nhiệm vụ Day 26:**

1. Điền các chỗ trống `___` trong code thực hành ở trên.
2. Tạo project Spring Boot 3 `auction-api` và hoàn thiện `AuctionController` như **Bước 1** trong phần thực hành.
3. Dùng lệnh `curl` xác nhận API `/api/health` và `/api/auctions` hoạt động tốt.
4. Tạo interface `BidRepository` và một implementation `InMemoryBidRepository` (dùng `ConcurrentHashMap` lưu bid theo `auctionId`). Đánh dấu `@Repository`.
5. Tạo `AuctionService` (`@Service`) **nhận `BidRepository` qua constructor**. Thêm method `placeBid(long auctionId, String bidder, long amount)` ghi bid vào repository.
6. Giải quyết **CHALLENGE** ở Bước 4: Implement `VipPricingStrategy` và xử lý conflict khi có 2 bean cùng kiểu.
7. Ghi vào `notes/day-26.md`: trả lời "IoC khác việc tự `new` object ở chỗ nào?" và "Bean trong Spring tương ứng với khái niệm gì bên Laravel?".

> 🎯 Tiêu chí đạt: API HTTP trả về kết quả đúng khi gọi `curl`, `placeBid` lưu được bid vào Map, và giải thích được bẫy NoUniqueBeanDefinitionException.
...
- [ ] Dựng thành công app Spring Boot cơ bản và gọi HTTP bằng `curl`
- [ ] Hoàn thành Challenge: xử lý conflict nhiều bean không dùng @Qualifier
- [ ] Hoàn thành Mini Project Day 26
