# Day 32 - Transactions (Giao dịch & quản lý đồng thời ở tầng DB)
...
- Hiểu `@Transactional` của Spring hoạt động **bằng proxy** — và vì sao **self-invocation** lại làm nó "im lặng không chạy" (nối tiếp bẫy proxy đã gặp ở Day 35).
...
Bạn **không** tự gọi `connection.commit()`. Bạn chỉ gắn `@Transactional` lên method, Spring lo phần còn lại. Nhưng cơ chế bên trong là **AOP proxy** — chính xác là cái proxy bạn đã gặp ở Day 35:
...
> ⚠️ **Bẫy self-invocation (nối tiếp Day 35):** Vì transaction được kích hoạt **ở proxy**, nếu một method **trong cùng class** gọi method `@Transactional` khác **bằng `this`**, lời gọi đó **không đi qua proxy** → `@Transactional` **bị bỏ qua hoàn toàn, không báo lỗi**.
>
> ```java
> @Service
...
public class Auction {
    ...
    @___ // Điền annotation để bật Optimistic Locking
...
public class BidService {
    ...
    /**
     * Đặt giá. BẮT BUỘC dùng tham số của @Transactional để CHẮC CHẮN rollback mọi lỗi
     * (không tin vào mặc định "chỉ rollback RuntimeException").
     */
    @___(rollbackFor = ___.class) // Điền tham số để bắt Spring rollback cho TOÀN BỘ Exception, kể cả Checked Exception
    public void placeBid(Long auctionId, String bidder, long amount) {
        ...
        if (amount <= auction.getHighestBid()) {
            throw new IllegalStateException("Giá thấp quá");
        }
...
public class BidEventRecorder {
    ...
    /**
     * Dùng transaction RIÊNG. Dù transaction đặt giá ở ngoài
     * có rollback, bản ghi audit này VẪN được commit (connection độc lập).
     */
    @Transactional(propagation = Propagation.___) // Điền kiểu Propagation để MỞ Transaction MỚI HOÀN TOÀN thay vì dùng chung
...
public interface AuctionRepository extends JpaRepository<Auction, Long> {
    ...
    // Sinh ra "SELECT ... FOR UPDATE": khóa hàng ngay khi đọc.
    @___(LockModeType.___) // Điền annotation để bật chế độ Khóa bi quan (khóa để GHI)
    @Query("SELECT a FROM Auction a WHERE a.id = :id")
    Optional<Auction> findByIdForUpdate(@Param("id") Long id);
}
```
...
spring:
  ...
  datasource:
    ...
    hikari:
      ___: 10   # Nhớ: REQUIRES_NEW mượn thêm connection -> Điền config để Pool size đừng quá nhỏ
```

### Bước 6 — CHALLENGE: Lỗi "Self-Invocation" 

> 🏆 Yêu cầu:
> 1. Viết method `public void doSomethingA()` và KHÔNG dùng `@Transactional`.
> 2. Viết method `public void doSomethingB()` và CÓ dùng `@Transactional`. Nó thực hiện `save()` một user, sau đó `throw new RuntimeException("Cố tình lỗi")`.
> 3. Cả 2 method nằm chung trong class `UserService`.
> 4. Trong `doSomethingA()`, gọi `this.doSomethingB()`.
> 5. Chạy hàm A, và nhận ra dữ liệu VẪN ĐƯỢC LƯU, dù đáng lẽ `doSomethingB` bị lỗi và phải Rollback. Tại sao? Trả lời và khắc phục lỗi kinh điển này.
...
1. Thêm cột `@Version version` vào entity `Auction`. Bật `show-sql` và quan sát câu `UPDATE ... WHERE id=? AND version=?` (Bài 1).
2. Viết `BidService.placeBid` với `@Transactional(rollbackFor = Exception.class)` (Bài 2).
3. Đảm bảo config Hikari CP có đủ kết nối (Bài 5).
4. Thêm `recordAttempt` với `REQUIRES_NEW`: chứng minh bản ghi audit **vẫn còn** dù bid chính rollback (Bài 3).
5. Hoàn thành **CHALLENGE** ở Bước 6: Mô phỏng bẫy Self-Invocation và xử lý nó.

**Kết quả mong đợi:** Audit lưu thành công, Transaction hoạt động nhất quán, hiểu đúng cơ chế Rollback.
...
- [ ] Áp dụng Rollback cho mọi Exception
- [ ] Config Propagation REQUIRES_NEW cho tính năng ghi Log
- [ ] Hoàn thành Challenge: Sửa lỗi bẫy Self-Invocation (Sử dụng `@Lazy` hoặc gọi qua Interface).
