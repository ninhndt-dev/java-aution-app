...
___ class Bid { // Điền từ khóa cho lớp trừu tượng
    ...
    public ___ long effectivePrice(); // Điền từ khóa cho method không có thân hàm

    public String bidder() { return bidder; }
}

// Đặt giá thường: giá hiệu lực = giá đặt
class RegularBid ___ Bid { // Điền từ khóa kế thừa lớp
    RegularBid(String bidder, long base) { super(bidder, base); }
    ___ public long effectivePrice() { return base; }   // Điền annotation ghi đè, DYNAMIC dispatch
}
...
final class Bid {
    ...
    @Override public boolean equals(___ o) { // Điền lớp gốc của mọi đối tượng
        if (this == o) return true;
        if (!(o ___ Bid b)) return false; // Điền toán tử kiểm tra kiểu
        return id == b.id && amount == b.amount && Objects.equals(bidder, b.bidder);
    }

    // PHẢI dùng đúng các field đã dùng trong equals
    @Override public int ___() { return Objects.hash(id, bidder, amount); } // Điền tên method bắt buộc đi kèm với equals
