...
Spring Security **không** can thiệp vào controller của bạn. Nó hoạt động hoàn toàn ở **tầng Servlet Filter**, **trước** `DispatcherServlet` (nhớ Day 33: Filter đứng trước MVC).
...
@Service
public class AuctionUserDetailsService implements ___ { // Điền interface Spring dùng để nạp user
    ...
    @Override
    public UserDetails ___(String username) throws UsernameNotFoundException { // Điền tên method
...
@Configuration
@___   // Điền annotation bật tính năng phân quyền theo method như @PreAuthorize
public class SecurityConfig {

    @___ // Điền annotation tạo Bean
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            .authorizeHttpRequests(auth -> auth
    ...
    @Bean
    public PasswordEncoder passwordEncoder() {
        return new ___(); // Điền Class mã hoá mật khẩu khuyên dùng
    }
}
```
...
public class AuctionService {
    ...
    @___("hasRole('ADMIN')") // Điền annotation yêu cầu người dùng phải có ROLE_ADMIN trước khi chạy hàm
...
public ResponseEntity<BidDto> placeBid(
        ...
        ___ authentication) {   // Điền interface đại diện cho người dùng đã xác thực của Spring
...
public class RegistrationService {
    ...
    public void register(String username, String rawPassword) {
        String hash = passwordEncoder.___(rawPassword);  // Điền hàm mã hoá của PasswordEncoder
...
### Bước 6 — CHALLENGE: Authentication và Authorization bằng cURL

> 🏆 Yêu cầu:
> 1. Chạy ứng dụng.
> 2. Gọi cURL vào endpoint công khai (`GET /auctions`) -> Status 200 OK.
> 3. Gọi cURL vào endpoint cần xác thực (`POST /auctions/1/bids`) mà không kèm header Authorization -> Status 401 Unauthorized.
> 4. Gọi cURL có chứa Authentication dạng BasicAuth: `curl -u user:password ...` vào endpoint cần xác thực -> Status 200/201 Created.
> 5. Cố tình dùng user thường gọi endpoint của admin (`POST /auctions/1/close`) -> Status 403 Forbidden.
...
1. Tạo cấu trúc Authentication bao gồm `UserDetailsService` (Bài 1).
2. Tạo cấu hình Security bằng Spring Security 6 với `SecurityFilterChain` (Bài 2).
3. Triển khai phân quyền Method Level với `@PreAuthorize` (Bài 3).
4. Thực hành việc extract Context của User từ `Authentication` parameter trong Controller (Bài 4).
5. Mã hoá Password khi tạo User (Bài 5).
6. Hoàn thành **CHALLENGE** ở Bước 6: Test HTTP Response theo các phân quyền khác nhau.

> 🎯 Tiêu chí đạt: App chạy đúng SecurityFilterChain. Phân biệt được khi nào bị 401 và khi nào bị 403 thông qua test curl. Cấu hình bảo mật thành công cho RESTful.
...
- [ ] Cấu hình Spring Security 6 Bean
- [ ] Bật `@EnableMethodSecurity` và sử dụng
- [ ] Extract User Context trong Controller
- [ ] Hoàn thành Challenge: Test Authentication & Authorization với cURL
