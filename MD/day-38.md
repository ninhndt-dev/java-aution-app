...
7. **`refresh()` context** — bước nặng nhất:
   ...
   - Chạy **Auto Configuration** (Day 37).
...
debug: true   # in cả Condition Evaluation Report (Day 37)
```
...
Dùng `CommandLineRunner` để chạy tự động một đoạn code dùng để tạo dữ liệu giả lập lúc khởi động.
...
@Component
@___(2)                 // Điền annotation quy định thứ tự chạy, số nhỏ chạy trước
@___("dev")             // Điền annotation CHỈ kích hoạt ở môi trường dev, không seed production
public class AuctionSeedRunner implements ___ { // Điền Interface runner đơn giản
...
public ___ StartupInfoRunner ___ ApplicationRunner { // Điền từ khóa khai báo lớp và kế thừa interface
...
@___ // Điền annotation đánh dấu Component
public ___ ReadyLogger { // Điền từ khóa khai báo lớp

    @___(ApplicationReadyEvent.class) // Điền annotation nghe sự kiện
    public void onReady() {
...
### Bước 6 — CHALLENGE: Lỗi trong CommandLineRunner (Fail-Fast)

> 🏆 Yêu cầu:
> 1. Trong hàm `run` của `AuctionSeedRunner`, hãy thử ném ra một `RuntimeException("Lỗi thử nghiệm Seed")` và chạy lại ứng dụng.
> 2. Bạn quan sát thấy ứng dụng bị gì? (Gợi ý: Nó có khởi động xong không?).
> 3. Hãy comment dòng code gây lỗi lại và chạy tiếp. Bài học ở đây là cẩn thận với exception trong các khối `*Runner` vì nó sẽ làm crash luôn app.
...
2. Thêm `StartupInfoRunner` đọc tham số `--reset-data` và `--env`, chạy trước Seed runner (Bài 2).
3. Thêm `ReadyLogger` nghe `ApplicationReadyEvent` in dòng "Auction API đã sẵn sàng" (Bài 3).
4. Cấu hình `banner.txt` hiển thị logo lúc khởi động (Bài 4).
5. Hoàn thành **CHALLENGE** ở Bước 6: Thử nghiệm ném Exception trong `CommandLineRunner` và xác nhận ứng dụng chết đứng (Fail-Fast).
6. Thử chạy cấu hình Lazy Init và so sánh thời gian khởi động trong log so với ban đầu.

> 🎯 Tiêu chí đạt: App chạy đúng thứ tự: Startup Info -> Seed Runner -> Application Ready. Đã thử test lỗi Fail-Fast. Đã test bật banner.
...
- [ ] Implement `CommandLineRunner` và `ApplicationRunner` (Bước 1, 2)
- [ ] Cấu hình chạy ưu tiên với `@Order`
- [ ] Dùng `ApplicationReadyEvent` log sẵn sàng
- [ ] Hoàn thành Challenge: Kiểm tra Crash khi lỗi Runner
