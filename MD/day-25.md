...
public ___ Bid(String bidder, long amount, long timestamp) { // Điền từ khóa khai báo class bất biến
    public Bid {
    ...
    public static Bid of(String bidder, long amount) {
        return new ___(bidder, amount, System.nanoTime()); // Điền class Bid
    }
}
```
...
public class Auction {
    ...
    private ___ boolean closed = false; // Điền từ khóa visibility

    // Giá cao nhất: read-modify-write có điều kiện → AtomicReference + CAS (Day 19)
    private final ___<Bid> highest; // Điền class wrapper nguyên tử

    // Đếm bid hợp lệ — ghi rất nhiều → LongAdder (Day 24)
    private final ___ acceptedCount = new ___(); // Điền class đếm hiệu suất cao
    private final ___ rejectedCount = new ___();
    ...
    public BidResult placeBid(Bid newBid) {
        ...
        Bid current;
        do {
            current = highest.___();               // Điền phương thức snapshot giá cao nhất hiện tại
            if (newBid.amount() <= current.amount()) {
                rejectedCount.increment();
                return BidResult.REJECTED_LOW;     // không cao hơn → từ chối
            }
            // CAS fail = ai đó vừa nâng giá → lặp lại với giá trị mới nhất
        } while (!highest.___(current, newBid)); // Điền phương thức CAS
