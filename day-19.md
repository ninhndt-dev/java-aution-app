    ...
    public static void main(String[] args) throws Exception {
        ...
        try (ExecutorService pool = Executors.newFixedThreadPool(THREADS)) {
            ...
            for (int t = 0; t < THREADS; t++) {
                pool.submit(() -> {
                    for (int i = 0; i < LOOPS; i++) {
                        safeCount.incrementAndGet();   // ← nguyên tử (CAS bên trong)
                    }
                    done.countDown();
                });
            }
...
public class ConditionalUpdateDemo {
    ...
    static void updateMaxManual(int candidate) {
        ...
        } while (!maxSeen.compareAndSet(cur, candidate)); // fail (ai chen ngang) → lặp lại
    }

    // Cách 2: updateAndGet — gọn hơn, tự lặp CAS bên trong
    static void updateMaxLambda(int candidate) {
        maxSeen.updateAndGet(cur -> Math.max(cur, candidate));
    }
        ...
        try (ExecutorService pool = Executors.newFixedThreadPool(50)) {
            ...
            for (int i = 0; i < 50; i++) {
                pool.submit(() -> {
                    ...
                    cache.computeIfAbsent("config", ComputeIfAbsentDemo::expensiveCreate);
                    done.countDown();
                });
            }
            done.await();
...
public class SafeAuction {
    ...
    public boolean placeBid(Bid newBid) {
        ...
        } while (!highestBid.compareAndSet(current, newBid));
