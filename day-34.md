...
> 💡 Quy tắc nhớ: việc cần làm **trước cả Spring MVC** (CORS, security, đọc/sửa body thô) → **Filter**. Việc cần biết "controller nào sắp chạy" → **Interceptor**. Spring Security hoạt động hoàn toàn ở tầng **Filter** (xem Day 35).
...
- **DTO + MapStruct/manual mapping**: chuẩn production là tách entity khỏi lớp web. Day 36-37 ta dùng JPA entity, nhưng controller luôn nói chuyện qua DTO.
...
**Nhiệm vụ Day 34:**
1. Tạo `AuctionController` với CRUD: `GET /auctions`, `GET /auctions/{id}`, `POST /auctions`, `DELETE /auctions/{id}`.
2. Thêm endpoint đặt giá `POST /auctions/{id}/bids` nhận `PlaceBidRequest` (có `@Valid`).
3. Tạo `BidTooLowException` và `AuctionNotFoundException` (exception nghiệp vụ).
4. Viết `GlobalExceptionHandler` (`@RestControllerAdvice`) trả: 409 cho `BidTooLowException`, 404 cho not found, 400 cho lỗi validation — với body `ApiError` chuẩn.
5. Thêm `TimingInterceptor` đo thời gian mỗi request `/auctions/**`.
6. Dùng **DTO** (record) cho request/response, **không** trả entity trực tiếp.
7. Test bằng `curl`/Postman: đặt giá thấp hơn giá hiện tại → nhận 409 với code `BID_TOO_LOW`.
