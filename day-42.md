...
class BidServiceTest {
    ...
    @Mock AuctionRepository repo;
    @Mock BidEventRecorder recorder;
    @InjectMocks BidService service;        // BidService(repo, recorder)
...
### Bước 3 — `@WebMvcTest` cho controller

```java
// File: AuctionControllerTest.java
...
class AuctionControllerTest {
    ...
    @Autowired MockMvc mvc;
    @MockBean BidService bidService;          // service được mock, KHÔNG nạp tầng dưới
...
### Bước 4 — `@DataJpaTest` cho repository

```java
// File: AuctionRepositoryTest.java
...
@DataJpaTest
@Testcontainers
@org.springframework.test.context.junit.jupiter.SpringExtension  // (đi kèm @DataJpaTest)
class AuctionRepositoryTest {

    // @ServiceConnection: Spring Boot tự nối datasource tới container (không cần @DynamicPropertySource)
    ...
    void save_and_find_with_version() {
        ...
        assertThat(found.getItemName()).isEqualTo("Đồng hồ cổ");
    }
}
```
...
### Bước 5 — Integration test luồng đặt giá (concurrency)
...
1. **Unit test** `BidService` bằng Mockito: từ chối bid thấp (không lưu), chấp nhận bid cao (cập nhật + ghi audit), `@ParameterizedTest` cho biên giá.
2. **`@WebMvcTest`** `AuctionController`: bid hợp lệ → 201, bid thấp → 400, JSON body parse đúng.
3. **`@DataJpaTest`** + Testcontainers Postgres: lưu/đọc `Auction`, kiểm tra `@Version` khởi tạo 0 và tăng sau update.
4. **Integration test concurrency**: 200 thread đặt giá tăng dần qua `placeBidWithRetry`, khẳng định `highestBid` cuối = giá lớn nhất, **không lost update** (chứng minh optimistic lock Day 38 hoạt động trên DB thật).
5. (Tuỳ chọn) Test Kafka consumer idempotent: gửi cùng `eventId` hai lần, khẳng định analytics chỉ đếm một lần (Day 40).
