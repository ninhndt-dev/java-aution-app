...
@ConfigurationProperties(prefix = "auction.notify")
public class AuctionNotifyProperties {

    /** Bật/tắt module thông báo. Mặc định bật. */
    private boolean enabled = true;

    /** Kênh gửi: "log", "email", "sms". Mặc định ghi log. */
...
// Bản mặc định: chỉ ghi ra log. Trong production có thể đổi sang email/SMS.
public class LoggingAuctionNotifier implements AuctionNotifier {
...
@AutoConfiguration                                  // đây là một auto-config (Boot 3)
...
@ConditionalOnProperty(prefix = "auction.notify", name = "enabled",
                       havingValue = "true", matchIfMissing = true)
public class AuctionNotifyAutoConfiguration {

    @Bean
    @ConditionalOnMissingBean   // CHỈ tạo bean mặc định nếu người dùng CHƯA tự khai báo AuctionNotifier
    public AuctionNotifier auctionNotifier(AuctionNotifyProperties props) {
        // Ở đây có thể switch theo props.getChannel() để chọn impl khác nhau
...
**Bước 2.6 — Chứng minh "cho phép ghi đè":**

```java
// Nếu người dùng tự khai báo bean này, @ConditionalOnMissingBean khiến
// auto-config LÙI lại, không tạo LoggingAuctionNotifier nữa.
@Bean
public AuctionNotifier emailNotifier() {
    return message -> sendEmail("admin@auction.com", message);
}
```

> ✅ **Bài tập:** Đổi `auction.notify.enabled=false` trong `application.yml`, chạy `--debug`, và xác nhận `AuctionNotifyAutoConfiguration` rơi vào **Negative matches**.
...
**Nhiệm vụ Day 32:**
1. Tạo package `com.example.auction.notify` với `AuctionNotifier`, `LoggingAuctionNotifier`, `AuctionNotifyProperties`, `AuctionNotifyAutoConfiguration` như trên.
2. Khai báo file `AutoConfiguration.imports` đúng đường dẫn.
3. Inject `AuctionNotifier` vào `BidService` mà **không** khai báo bean thủ công — xác nhận nó vẫn chạy (nhờ auto-config).
4. Chạy app với `--debug`, chụp lại phần report cho thấy `AuctionNotifyAutoConfiguration` ở **Positive matches** kèm điều kiện đã thỏa.
5. Thử khai báo một `@Bean AuctionNotifier` riêng (gửi email giả) và xác nhận `@ConditionalOnMissingBean` khiến auto-config **lùi lại**.
6. Đặt `auction.notify.enabled=false`, chạy lại, xác nhận config rơi vào **Negative matches**.
...
- [ ] Tự viết được một `@AutoConfiguration` dùng `@ConditionalOnMissingBean` + `@ConditionalOnProperty`
- [ ] Chạy `--debug` và đọc hiểu Condition Evaluation Report
- [ ] Đối chiếu đúng với Package Auto-Discovery của Laravel
- [ ] Hoàn thành Mini Project module thông báo auto-config
