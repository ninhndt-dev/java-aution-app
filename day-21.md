...
public class VisibilityFixed {
    private static volatile boolean stopRequested = false;   // ← chỉ thêm volatile
...
public class VolatileNotAtomic {
    static volatile int volatileCount = 0;             // volatile nhưng VẪN không atomic
            ...
            for (int t = 0; t < THREADS; t++) {
                pool.submit(() -> {
                    for (int i = 0; i < LOOPS; i++) {
                        ...
                        atomicCount.incrementAndGet();  // ← nguyên tử (CAS) → luôn đúng
                    }
                    done.countDown();
                });
            }
...
public class AuctionSession {
    ...
    public boolean placeBid(Bid bid) {
        ...
        } while (!highest.compareAndSet(cur, bid));     // CAS → không mất bid (Day 19)
        return true;
    }
...
public class AuctionSessionDemo {
    public static void main(String[] args) throws Exception {
        ...
        timer.schedule(session::close, 500, TimeUnit.MILLISECONDS);
