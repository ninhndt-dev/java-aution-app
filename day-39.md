...
> ⚠️ `@Cacheable` cũng là **proxy-based** như `@Transactional` (Day 30, Day 38). **Self-invocation cũng nuốt cache!** Gọi method `@Cacheable` bằng `this` từ trong cùng class → cache không hoạt động.
...
@Configuration
@EnableCaching   // ⭐ Bật toàn bộ cơ chế @Cacheable/@CachePut/@CacheEvict
...
### Bước 3 — `@Cacheable` / `@CachePut` / `@CacheEvict` cho thông tin item
...
public class AuctionQueryService {
    ...
    @Cacheable(value = "auctions", key = "#id", unless = "#result == null")
    public AuctionDto getAuction(Long id) {
        // log này CHỈ in khi miss (method bị bỏ qua khi hit) -> bằng chứng cache chạy
        System.out.println("⛏️  MISS -> query DB cho auction " + id);
        return repo.findById(id).map(AuctionDto::from).orElse(null);
    }

    // UPDATE: luôn chạy method, rồi GHI ĐÈ cache bằng kết quả mới (giữ cache đồng bộ)
    @CachePut(value = "auctions", key = "#result.id")
...
public class BidLeaderboardService {
     ...
     * -> mỗi bidder chỉ giữ giá CAO NHẤT của họ (ta luôn ghi giá mới >= cũ).
     */
    public void recordBid(Long auctionId, String bidder, long amount) {
        redis.opsForZSet().add(key(auctionId), bidder, amount);   // O(log N), atomic
    }
...
### Bước 5 — Test cache hit/miss

```java
@Test
void caching_should_skip_db_on_second_call() {
    queryService.getAuction(42L);   // in "MISS -> query DB"
    queryService.getAuction(42L);   // KHÔNG in gì -> trả từ cache
    // -> chỉ thấy 1 dòng MISS trong log, chứng minh @Cacheable hoạt động
}
```
...
- **Đừng cache thứ phải đúng tuyệt đối thời gian thực** (giá đang chốt giao dịch). Cache phục vụ đọc; ghi quan trọng vẫn qua DB + locking (Day 38).
...
1. Thêm `@Cacheable` cho `getAuction(id)` với TTL 10s; viết test chứng minh lần gọi thứ 2 **không** chạm DB (chỉ 1 dòng "MISS" trong log).
2. Khi đặt giá thành công (nối tiếp `BidService` Day 38), gọi `BidLeaderboardService.recordBid(auctionId, bidder, amount)` để cập nhật ZSet.
3. Tạo endpoint `GET /auctions/{id}/leaderboard?top=10` trả top N người trả giá cao nhất (giá + bidder) lấy từ Redis, **không chạm DB**.
4. Khi `closeAuction`, dùng `@CacheEvict` xoá cache phiên và `clear()` luôn key leaderboard.
5. (Tuỳ chọn) Thêm jitter TTL và bật `@Cacheable(sync = true)` để chống stampede ở phiên hot; đo hit ratio.
