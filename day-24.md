...
public class FrequencyCountDemo {
    public static void main(String[] args) throws Exception {
        ...
        ConcurrentHashMap<String, Integer> bidsPerUser = new ConcurrentHashMap<>();
        // Tổng số bid toàn hệ thống — ghi rất nhiều → LongAdder
        LongAdder totalBids = new LongAdder();
            ...
            for (int t = 0; t < 8; t++) {
                pool.submit(() -> {
                    for (int i = 0; i < 100_000; i++) {
                        ...
                        bidsPerUser.merge(u, 1, Integer::sum);   // nguyên tử: đọc-cộng-ghi value
                        totalBids.increment();                   // nguyên tử, tối ưu ghi nhiều
                    }
                    done.countDown();
                });
            }
...
public class ProducerConsumerDemo {
    ...
    public static void main(String[] args) throws Exception {
        ...
        BlockingQueue<Bid> queue = new ArrayBlockingQueue<>(100);
        int CONSUMERS = 3;
        ...
        try (ExecutorService pool = Executors.newFixedThreadPool(CONSUMERS + 1)) {
            ...
            pool.submit(() -> {
                try {
                    for (int i = 1; i <= 1000; i++) {
                        queue.put(new Bid(i, i * 100L));    // CHẶN nếu queue đầy
                    }
            ...
            for (int c = 0; c < CONSUMERS; c++) {
                pool.submit(() -> {
                    try {
                        while (true) {
                            Bid bid = queue.take();          // CHẶN nếu queue rỗng
