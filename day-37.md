# Day 37 - Hibernate Persistence Context
...
public class AuctionService {
    ...
    public void renameAuction(Long id, String newTitle) {
        Auction auction = auctionRepository.findById(id).orElseThrow();  // managed
        auction.setTitle(newTitle);
        // KHÔNG gọi save(). Khi method kết thúc → commit → Hibernate tự:
...
public void proveIdentityMap(Long id) {
    ...
    System.out.println("Cùng object? " + (a1 == a2));  // true (chỉ 1 query SQL)
}
```
...
public void demoNPlusOne() {
    List<Auction> auctions = auctionRepository.findAll();   // query 1
    for (Auction a : auctions) {
        // mỗi lần getBids() lần đầu → 1 query lazy => tổng N+1 query
        System.out.println(a.getTitle() + " có " + a.getBids().size() + " lượt đặt giá");
    }
}
```
Bật `show-sql` và đếm số câu `SELECT` để **tận mắt** thấy N+1.

### Bài 4 — Sửa N+1 bằng JOIN FETCH

```java
public interface AuctionRepository extends JpaRepository<Auction, Long> {

    // DISTINCT vì JOIN FETCH collection làm nhân dòng
    @Query("SELECT DISTINCT a FROM Auction a LEFT JOIN FETCH a.bids")
...
public void fixedWithFetch() {
    List<Auction> auctions = auctionRepository.findAllWithBids();  // 1 query duy nhất
    for (Auction a : auctions) {
...
### Bài 5 — Sửa N+1 bằng `@EntityGraph`

```java
public interface AuctionRepository extends JpaRepository<Auction, Long> {

    @EntityGraph(attributePaths = "bids")     // nạp sẵn bids cùng auction
    List<Auction> findByStatus(AuctionStatus status);
}
```

### Bài 6 — Batch size (lưới an toàn)

```yaml
# application.yml
spring:
  jpa:
    open-in-view: false      # tắt OSIV: ép fetch tường minh, tránh query ẩn ở view
    properties:
      hibernate.default_batch_fetch_size: 20   # gom lazy load theo lô 20
```

> ✅ **Bài tập tự giải thích:** Chạy Bài 3 rồi Bài 4, so sánh số dòng `SELECT` trong log. Vì sao `JOIN FETCH` collection cần `DISTINCT`? Vì sao không nên `JOIN FETCH` collection kèm `Pageable`?
...
2. Sửa bằng hai cách: (a) `@Query ... JOIN FETCH a.bids` (nhớ `DISTINCT`); (b) `@EntityGraph(attributePaths="bids")`. So sánh số query trước/sau.
3. Tắt `open-in-view`, map entity → DTO (`AuctionWithBidsDto`) **trong** tầng service (trong transaction) để tránh `LazyInitializationException`.
4. Viết `renameAuction(id, title)` minh họa dirty checking (không gọi `save()`), xác nhận log có `UPDATE`.
5. Thêm `@Transactional(readOnly = true)` cho các method chỉ đọc.
6. (Tùy chọn) Bật `hibernate.default_batch_fetch_size=20` và quan sát N+1 giảm thành ~N/20 query.
7. Ghi chú: liệt kê 3 điểm Eloquent khác Hibernate mà bạn vừa "thấm" (dirty checking, identity map, lazy exception).
...
- [ ] Hoàn thành Mini Project tối ưu truy vấn tránh N+1
