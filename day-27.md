...
### Bước 1 — Constructor injection: wiring `BidService` vào `AuctionController`
...
public class BidService {
    ...
    // 1 constructor -> không cần @Autowired (Spring 4.3+)
...
public class AuctionController {
    ...
    private final BidService bidService; // phụ thuộc bắt buộc, final

    // Spring tiêm BidService vào qua constructor (constructor injection)
...
public interface PricingStrategy {
    long minIncrement(long currentPrice); // bước giá tối thiểu mỗi lần đặt
}
```
...
@Primary                          // <-- bean mặc định khi nhập nhằng
@Component("standardPricing")
public class StandardPricingStrategy implements PricingStrategy {
    public long minIncrement(long currentPrice) {
        return Math.max(10_000, currentPrice / 100); // 1% hoặc tối thiểu 10k
    }
}
```
...
public class VipPricingStrategy implements PricingStrategy {
    public long minIncrement(long currentPrice) {
        return Math.max(1_000, currentPrice / 1000); // VIP bước giá nhỏ hơn
    }
}
```
...
public class AuctionService {
    ...
    // Có 2 bean PricingStrategy -> @Qualifier chỉ rõ muốn bean "vipPricing"
    // Nếu BỎ @Qualifier, Spring sẽ dùng bean @Primary (standardPricing)
    public AuctionService(@Qualifier("vipPricing") PricingStrategy pricing) {
        this.pricing = pricing;
    }

    public long nextValidBid(long currentPrice) {
        return currentPrice + pricing.minIncrement(currentPrice);
    }
}
```

### Bước 3 — Inject `List`/`Map` cho chuỗi validator

```java
// File: BidValidator.java
public interface BidValidator {
    void validate(Bid bid); // ném exception nếu không hợp lệ
}
```

```java
// File: BidValidatorChain.java
import org.springframework.stereotype.Service;
import java.util.List;

@Service
public class BidValidatorChain {
    private final List<BidValidator> validators; // Spring tiêm TẤT CẢ bean BidValidator

    public BidValidatorChain(List<BidValidator> validators) {
        this.validators = validators;
    }

    public void validateAll(Bid bid) {
        // chạy lần lượt mọi validator (thứ tự theo @Order nếu có)
        validators.forEach(v -> v.validate(bid));
    }
}
```

> ✅ **Bài tập tự giải thích:** Thêm `@Order(1)` cho validator kiểm tra "số tiền > 0" và `@Order(2)` cho validator "phải cao hơn giá hiện tại". Quan sát thứ tự chúng chạy trong `List`. Vì sao thứ tự lại quan trọng ở đây?
...
> Hôm nay ta **wiring toàn bộ tầng service đấu giá bằng constructor injection**, và xử lý nhiều chiến lược giá.

**Nhiệm vụ Day 27:**

1. `BidService` (`@Service`) nhận `BidRepository` qua constructor (giữ từ Day 26).
2. `AuctionService` (`@Service`) nhận `BidService` + một `PricingStrategy` qua constructor.
3. Tạo **2 bean** `PricingStrategy`: `StandardPricingStrategy` (`@Primary`) và `VipPricingStrategy`. Trong `AuctionService` dùng `@Qualifier` để chọn rõ ràng; thử bỏ `@Qualifier` xem Spring chọn `@Primary`.
4. Tạo `BidValidatorChain` nhận `List<BidValidator>` (ít nhất 2 validator: số tiền dương, cao hơn giá hiện tại). Dùng `@Order` để định thứ tự.
5. Tạo `AuctionController` (`@RestController`) nhận `AuctionService` qua constructor, expose `POST /api/auctions/{id}/bids`.
6. Viết **một unit test thuần** cho `BidService` bằng cách `new BidService(mockRepo)` với một `BidRepository` giả — chứng minh constructor injection giúp test dễ ra sao.
...
> 🎯 Tiêu chí đạt: app khởi động, đặt bid qua HTTP chạy được, unit test `BidService` xanh, và bạn giải thích được vai trò `@Primary`/`@Qualifier`.
...
- [ ] Hoàn thành Mini Project Day 27 (wiring service đấu giá + unit test thuần)
