...
spring:
  kafka:
    ...
    producer:
      acks: all                       # bền nhất: chờ mọi replica ISR ghi xong
...
// record: bất biến, serialize JSON gọn. eventId để consumer idempotent.
...
public class BidEventPublisher {
    ...
    public void publish(Long auctionId, String bidder, long amount) {
        ...
        kafkaTemplate.send(TOPIC, auctionId.toString(), event)
            .whenComplete((result, ex) -> {
                if (ex != null) {
                    // ghi lỗi gửi (production: cân nhắc outbox pattern để không mất event)
...
### Bước 4 — Consumer: `@KafkaListener` cập nhật analytics/thông báo
...
public class BidAnalyticsConsumer {
    ...
    // Production: lưu vào Redis/DB có TTL, không phải in-memory.
    private final Set<String> processed = ConcurrentHashMap.newKeySet();

    @KafkaListener(topics = BidEventPublisher.TOPIC, groupId = "auction-analytics")
    public void onBidPlaced(BidPlacedEvent event) {
        ...
        if (!processed.add(event.eventId())) {
            return;   // đã xử lý rồi -> bỏ qua (đảm bảo idempotent)
        }
        // Cập nhật analytics: đếm số bid/phút, tổng giá trị, top bidder...
...
class BidNotificationConsumer {
    ...
    @KafkaListener(topics = BidEventPublisher.TOPIC, groupId = "auction-notification")
    public void notifyOutbid(BidPlacedEvent event) {
        // Gửi thông báo "có người trả giá cao hơn bạn"
...
> 💡 Hai consumer trên thuộc **hai group khác nhau** (`auction-analytics`, `auction-notification`) → **cả hai đều nhận** mọi sự kiện, mỗi group giữ offset riêng. Đây chính là sức mạnh "nhiều consumer độc lập" mà queue truyền thống khó làm.

### Bước 5 — Nối vào nghiệp vụ đặt giá

```java
// Trong BidService.placeBid (Day 38), SAU khi commit transaction DB thành công:
@Transactional
public void placeBid(Long auctionId, String bidder, long amount) {
    // ... cập nhật DB như Day 38 ...
    // Lưu ý: publish nên xảy ra SAU commit (dùng @TransactionalEventListener
    //        hoặc outbox pattern) để không phát event cho transaction đã rollback.
...
1. Tạo topic `bid-placed`. Định nghĩa `BidPlacedEvent` (có `eventId`, `auctionId`, `bidder`, `amount`, `placedAt`).
2. Trong `BidService.placeBid`, **sau khi commit**, publish `BidPlacedEvent` với **key = auctionId** (đảm bảo thứ tự theo phiên).
3. Viết `BidAnalyticsConsumer` (group `auction-analytics`): đếm số bid mỗi phiên, **idempotent theo `eventId`**.
4. Viết `BidNotificationConsumer` (group `auction-notification`) đọc **cùng topic độc lập**: in thông báo "có giá mới". Chứng minh cả hai group đều nhận đủ sự kiện.
5. (Tuỳ chọn) Cấu hình `@RetryableTopic` + DLT cho consumer; mô phỏng lỗi để thấy message rơi vào DLT thay vì kẹt partition.

**Kết quả mong đợi:** đặt giá xong, hệ thống phát một sự kiện; analytics và notification xử lý **độc lập** cùng sự kiện đó; bid của cùng một phiên được xử lý **đúng thứ tự** (cùng partition); gửi lại trùng không làm sai số liệu (idempotent).
