...
public class FutureVsCompletable {
    ...
    public static void main(String[] args) throws Exception {
        try (ExecutorService pool = Executors.newFixedThreadPool(3)) {
            ...
            CompletableFuture<Long> ca = CompletableFuture.supplyAsync(() -> slow("C", 300, 10), pool);
            CompletableFuture<Long> cb = CompletableFuture.supplyAsync(() -> slow("D", 300, 20), pool);
            ca.thenCombine(cb, Long::sum)
              .thenAccept(s -> System.out.println("CF sum=" + s
...
public class PipelineDemo {
    ...
    static CompletableFuture<User>  loadUser(long id, Executor ex) {
        return CompletableFuture.supplyAsync(() -> { sleep(100); return new User(id, "U" + id); }, ex);
    }
    static CompletableFuture<Order> loadLatestOrder(long userId, Executor ex) {
        return CompletableFuture.supplyAsync(() -> { sleep(100); return new Order(9, userId, 500); }, ex);
    }

    public static void main(String[] args) {
        try (ExecutorService pool = Executors.newFixedThreadPool(4)) {
            loadUser(42, pool)
                .thenApply(user -> { System.out.println("Có user: " + user.name()); return user; })  // biến đổi
                .thenCompose(user -> loadLatestOrder(user.id(), pool))   // PHỤ THUỘC → compose (flatMap)
                .thenAccept(order -> System.out.println("Đơn mới nhất tổng = " + order.total()))
                .join();
        }
    }
    ...
    public static void main(String[] args) {
        try (ExecutorService pool = Executors.newFixedThreadPool(8)) {
            ...
            List<CompletableFuture<Long>> futures = ids.stream()
                .map(id -> CompletableFuture.supplyAsync(() -> fetchPrice(id), pool))   // 10 lời gọi song song
                .toList();

            CompletableFuture<List<Long>> allPrices =
                CompletableFuture.allOf(futures.toArray(new CompletableFuture[0]))      // đợi tất cả
                    .thenApply(v -> futures.stream().map(CompletableFuture::join).toList());  // gom kết quả
