# Day 30 - Proxy (Cơ chế Proxy của Spring)
...
public class TimedProxyDemo {
    ...
    // Target THẬT
    static class RealBidService implements BidService {
    ...
    static class TimingHandler implements InvocationHandler {
        ...
        public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
            ...
            try {
                Object result = method.invoke(target, args); // chuyển tiếp xuống target
                return result;
            } finally {
    ...
    public static void main(String[] args) {
        ...
        BidService proxy = (BidService) Proxy.newProxyInstance(
                ...
                new Class<?>[]{ BidService.class },   // ⚠️ JDK proxy CẦN interface
...
public class BidService {
    ...
    public void placeBidWrong(Bid bid) {
        this.persist(bid); // this = target, không phải proxy
    }

    @Transactional
    public void persist(Bid bid) {
        bidRepository.insert(bid);
        if (bid.amount() < 0) throw new IllegalStateException("Bid âm -> đáng lẽ phải rollback");
        // Khi gọi qua placeBidWrong: KHÔNG có transaction -> KHÔNG rollback!
    }
}
```

```java
// ✅ CÁCH ĐÚNG #1: tách persist sang bean riêng
@Service
public class BidPersister {
    private final BidRepository repo;
    public BidPersister(BidRepository repo) { this.repo = repo; }

    @Transactional   // gọi từ BidService -> ĐI QUA PROXY của BidPersister -> @Transactional ĂN
    public void persist(Bid bid) {
        repo.insert(bid);
        if (bid.amount() < 0) throw new IllegalStateException("rollback đúng kỳ vọng");
    }
}

@Service
public class BidServiceFixed {
    private final BidPersister persister;
    public BidServiceFixed(BidPersister persister) { this.persister = persister; }

    public void placeBid(Bid bid) {
        persister.persist(bid); // gọi qua proxy của bean khác -> transaction kích hoạt
    }
}
```

```java
// ✅ CÁCH ĐÚNG #2: self-injection (ít sạch hơn nhưng giữ trong 1 class)
@Service
public class BidServiceSelf {
    private final BidRepository repo;

    @org.springframework.beans.factory.annotation.Autowired
    @org.springframework.context.annotation.Lazy
    private BidServiceSelf self;  // self = PROXY (không phải this)

    public BidServiceSelf(BidRepository repo) { this.repo = repo; }

    public void placeBid(Bid bid) {
        self.persist(bid);  // gọi QUA proxy -> @Transactional ăn
    }

    @Transactional
    public void persist(Bid bid) { repo.insert(bid); }
}
```

> ✅ **Bài tập tự giải thích:** Chạy `placeBidWrong` với một bid âm trong khi có một bid hợp lệ trước đó cùng "đơn vị công việc". Vì transaction không kích hoạt, bid hợp lệ **không rollback**. Sau đó chạy `BidPersister.persist` để thấy rollback đúng. Vì sao chỉ khác ở chỗ "gọi từ đâu"?
...
> Hôm nay ta hiểu **vì sao phải tách bean** để `@Transactional` ăn trong luồng đặt giá — kiến thức cứu bạn khỏi bug "mất tiền không rollback".

**Nhiệm vụ Day 30:**

1. Tự viết một JDK dynamic proxy (`Proxy.newProxyInstance` + `InvocationHandler`) bọc một `BidService` interface, đo thời gian mỗi lời gọi — in ra class proxy (`$Proxy...`).
2. Tạo `BidService.placeBidWrong(bid)` gọi nội bộ `this.persist(bid)` (`@Transactional`). Mô phỏng một bid lỗi và **chứng minh không rollback** (self-invocation phá transaction).
3. Tách `persist` sang bean `BidPersister` (`@Transactional`) rồi inject vào `BidService`. Lặp lại kịch bản lỗi → lần này **rollback đúng**.
4. (Mở rộng) Thử cách self-injection (`@Autowired @Lazy private BidService self;`) và so sánh.
...
> 🎯 Tiêu chí đạt: bạn tái hiện được bug self-invocation, sửa được bằng tách bean, và giải thích chính xác vai trò proxy.
...
- [ ] Tự viết được một JDK dynamic proxy minh họa
- [ ] Tái hiện và sửa được bẫy self-invocation phá `@Transactional`
- [ ] Hiểu vì sao `@Async`/`@Cacheable` cũng dính bẫy này
- [ ] Đối chiếu với `__call` của PHP và transaction tường minh của Laravel
- [ ] Hoàn thành Mini Project Day 30 (proxy + self-invocation demo)
